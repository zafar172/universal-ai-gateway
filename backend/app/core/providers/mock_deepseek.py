...
class MockDeepseekProvider(BaseProvider):
    def __init__(self, api_key: str = "mock"):
        self._name = "mock_deepseek"
        self.model_metadata = [ProviderMetadata(model_id="deepseek-coder-v2", provider_name=self._name, cost_per_1k_input=0.00014, cost_per_1k_output=0.00028, speed_tier="very_fast", reasoning_tier="high", context_window=128000)]
    @property
    ...
    async def generate_completion(self, request: CompletionRequest) -> CompletionResponse:
        return CompletionResponse(id="1", model=request.model, provider=self._name, content=f"[Deepseek]: Ready.", finish_reason="stop", usage={"prompt_tokens":10,"completion_tokens":20,"total_tokens":30}, cost_usd=0.0, latency_ms=100.0)
