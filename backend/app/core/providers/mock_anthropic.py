...
class MockAnthropicProvider(BaseProvider):
    def __init__(self, api_key: str = "mock"):
        ...
        self.model_metadata = [ProviderMetadata(model_id="claude-3-5-sonnet", provider_name=self._name, cost_per_1k_input=0.003, cost_per_1k_output=0.015, speed_tier="fast", reasoning_tier="expert", context_window=128000)]
    @property
    ...
    async def generate_completion(self, request: CompletionRequest) -> CompletionResponse:
        return CompletionResponse(id="1", model=request.model, provider=self._name, content=f"[Anthropic]: Ready.", finish_reason="stop", usage={"prompt_tokens":10,"completion_tokens":20,"total_tokens":30}, cost_usd=0.0, latency_ms=100.0)
